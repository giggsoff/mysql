...
docker_create_db_directories() {
	...
	# TODO /var/lib/mysql-files and /var/lib/mysql-keyring ?
	...
	for conf in \
		...
		slow-query-log-file \
	; do
		dir="$(dirname "$(mysql_get_config "$conf" "$@")")" # get directory contains file from config
		if [[ "$dir" = /* ]]; then
			dirs["$dir"]=1
		fi
	done
	...
	if [ "$user" = "0" ]; then
		...
		find "${!dirs[@]}" \! -user mysql -exec chown mysql '{}' +
	fi
}
